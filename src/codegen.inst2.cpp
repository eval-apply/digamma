...
void
codegen_t::emit_subr_num_eq_2(context_t& ctx, scm_obj_t inst, scm_subr_t subr)
{
    DECLEAR_CONTEXT_VARS;
    DECLEAR_COMMON_TYPES;
...
//    auto ans = IRB.CreateAlloca(IntptrTy);
    ...
    BasicBlock* fixnum_false = BasicBlock::Create(C, "fixnum_false", F);
    auto fixnum_cond = IRB.CreateICmpNE(IRB.CreateAnd(IRB.CreateAnd(sp_minus_1, sp_minus_2), 1), VALUE_INTPTR(0));
    IRB.CreateCondBr(fixnum_cond, fixnum_true, fixnum_false);
    ...
    IRB.SetInsertPoint(fixnum_false);
...
codegen_t::emit_subr(context_t& ctx, scm_obj_t inst, scm_subr_t subr)
{
    ...
    if (argc == 2 && subr->adrs == subr_num_eq) {
        //puts("emit_subr_num_eq_2");
        emit_subr_num_eq_2(ctx, inst, subr);
        return;
    }
